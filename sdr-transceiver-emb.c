...
static void alex_write(uint32_t freq)
{
    uint16_t lpf = 0;
    static uint16_t i2c_alex_data = 0;
    uint16_t u16AlexDataTemp = 0;
  ...
  *0x03, 10 MHz, 3dB 15.5MHz
  0x04, 14 MHz, 3dB 15.5MHz
  *0x05, 18 MHz, 3dB 25MHz
  0x06, 21 MHz, 3dB 25MHz
  0x07, 24 MHz, 3dB 25MHz
  *0x08, 28 MHz, 3dB 35MHz
  *0x09, 50MHz, 3dB 55MHz
  */

    if (freq > 32000000)
        lpf = 0x09; /* 6 meters, 3dB 55MHz */
    else if (freq > 22000000)
        lpf = 0x08; /* 12/10 meters, 3dB 35MHz */
    else if (freq > 15000000)
        lpf = 0x05; /* 17/15 meters, 3dB 25MHz */
    else if (freq > 8000000)
        lpf = 0x03; /* 30/20 meters, 3dB 15.5MHz */
    else if (freq > 4500000)
        lpf = 0x02; /* 60/40 meters, 3dB 8.6MHz */
    else if (freq > 2400000)
        lpf = 0x01; /* 80 meters, 3dB 5MHz */
    else
        lpf = 0x00; /* 160 meters, 3dB 2.5MHz */

    u16AlexDataTemp = lpf;

    if (i2c_alex_data != u16AlexDataTemp)
    {
        i2c_alex_data = u16AlexDataTemp;
        ioctl(i2c_fd, I2C_SLAVE, ADDR_ALEX);
        i2c_write_addr_data16(i2c_fd, 0x02, i2c_alex_data);
    }
}
...
int main()
{
    int fd, running, filter, ptt, mux, i, j, k, l, size, indexj;
    uint16_t r;
    FILE *wisdom_file;
    ...
    // win_data should be loaded with a windowing function
    // fft_data, int32

    rx_rst = ((uint8_t *)(cfg + 0));
    sp_rst = ((uint8_t *)(cfg + 0)); // b2 and b3 (Reg shared with RX)
    ...
    sp_cntr = ((uint16_t *)(sts + 18)); // FFT data counter (similar as RX/TX)
    ...
    while (running)
    {
        if (FFT != FFT_OFF)
        {
            ...
            if (FFT_data_rdy != 0)
            {
                ...
                // rc is set to 1 if new data is available to draw
                if (rc)
                {
        ...
        while (1)
        {
            ...
            // Make sure we received 6bytes before processing it
            if (num < 6)
                break;
            ...
            case COM_SPEAKER_VOL: // output volume
                ...
                if (i2c_codec)
                {
                    ...
                    i2c_write_addr_data8(i2c_fd, 0x04, volume);
                    i2c_write_addr_data8(i2c_fd, 0x06, volume);
                }
                else
                {
            ...
            case COM_DEMOD_MODE:
                ...
                case DEMOD_AM:
                    ...
                    SetTXAMode(1, TXA_AM);
                    break;
                case DEMOD_FM: //  new 29.04.2017 WK
                    SetRXAMode(0, RXA_FM);
                    SetTXAMode(1, TXA_FM);
                    break;
                case DEMOD_SAM: // SAM
...
uint8_t Threshold3(int32_t value)
{
    ...
    if (flat_value > CW_ThresholdHigh)
    { // 200
    ...
    else if (flat_value < CW_ThresholdLow)
    { // 20
    ...
    else if (cw_receive_state != AREA1)
