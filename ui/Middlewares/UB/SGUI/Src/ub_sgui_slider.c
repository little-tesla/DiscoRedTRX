...
//--------------------------------------------------------------
uint32_t SGUI_SliderTouch(bool pressed, uint16_t x, uint16_t y, bool blocked)
{
  uint32_t ret_wert=0xffff;// return: Nummer des betätigten Sliders bzw. ffff
  uint32_t n;
  uint16_t x1,x2,y1,y2;
  uint8_t m=0, duration;
 // static bool old_status=false;
  ...
  if(pressed==false) {
    ...
    // update der Werte
    if(m==1) {
      duration=SGUI_SLIDER.ptr[n]->duration++;
	  ...
	  P_SGUI_SliderDraw(SGUI_SLIDER.ptr[n]);// neu zeichnen
    }
  }
  return (ret_wert);
}
