...
// Funktion : Hauptprogramm f�r RedPitaya Interface (Embedded Transceiver from Pavel Demin) DiscoRedTRX
// USART    : COM6
//  RX- Pin : PC7 ==> CN4 D0
//  TX- Pin : PC6 ==> CN4 D1
//--------------------------------------------------------------
...
char BufferFi[4][72] = {{
							"CQ CQ CQ DE DH1AKF DH1AKF DH1AKF PSE K ",
						},
						{
...
SBUTTON_t *oldptr; //Zeiger f�r Frequenzeingabe (Ziffernposition)
char oldpos;	   // numerischer Zeiger f�r Frequenzeingabe (Ziffernposition)
...
void DrawKeyboard(void)
{
	...
	for (px = 32; px <= 460; px += 45)
	{
		...
		SGUI_TextSetCursor(px + 18, py + 12);
		if (i == 8)
			SGUI_TextSetCursor(px + 10, py + 12);
		else
			SGUI_TextSetCursor(px + 18, py + 12);
		txt[0] = yxcv[i++];
		SGUI_TextPrintString((char *)&txt[0]);
...
void MX_GPIO_Init(void)
{
	...
	UB_ADC1_SINGLE_Init(); // f�r Test Batteriespannung *******************************************
...
uint32_t MakeFreq(char *freqText)
{ //"A 07.200.000" or "B 07.200.000"
	uint32_t help = 0;
	uint8_t i, z;
	for (i = 2; i < 12; i++)
	{
...
void MakeFrString(char *target, uint32_t frq, uint8_t length)
{ // result in char[] f1
	char i;
	int32_t freq;
...
void DrawGrid(void)
{
	...
	for (i = 0; i < 7; i++)
	{
		...
		if (i < 6)
			SGUI_TextPrintString((char *)&txt[i][0]);
	}
...
void Waterfall(void)
{
	...
	for (y = 33; y < 112; y++)
	{ // reserve place for the new values
	...
	for (i = -2; i < 43; i++)
	{
		...
		if (x < 455)
		{
			if ((hf % 5000) == 0)
			{
				...
				UB_Graphic2D_DrawRectDMA(x, 114, 1, 4, RGB_COL_BLACK);
...
{ // von TIM2 aufgerufen (Interrupt 5 kHz)
	if (StopCount > 18)
		StopCount = 9;
	if (++count == StopCount)
	...
	if (++count30 % 30 == 0)
		timer3++; // 166,66 Hz => 6 ms
	if (++count10 % 10 == 0)
		timer10++; // 500 Hz => 2 ms CW Decoder
	if (++count50 >= 50)
	{
		timer1++; // all 10 ms
		timer2++; // for cw keyer
...
{ // calculate median
	...
	{
		if (S_Val1[2] < S_Val1[0])
			return S_Val1[0];
		else if (S_Val1[2] < S_Val1[1])
			return S_Val1[1];
		else
			return S_Val1[2];
	}
	...
	{
		if (S_Val1[2] < S_Val1[1])
			return S_Val1[1];
		else if (S_Val1[2] > S_Val1[0])
			return S_Val1[0];
		else
			return S_Val1[2];
	}
...
void FrequencyShift(void)
{
	...
	if (cnt1 != 1)
		return; // first key touch, and after every 0,2 s --> new frequency
			...
			if (SelectSweep == 0)
			{
				...
				if (S_Val != -1)
				{
					...
					if (FFT != 0)
					{
						...
						if ((touchx > 5) && (touchy > 36) && (touchy < 158))
							FrequencyShift();
						else
						{
							tim1 = timer1;
				...
				if (ModeNr < 2)
				{
					...
					{ //2 ms
						...
						if ((UALabel != 86) && (UALabel != 89))
							CW_TIMER_EVENT(); // nur Zeit z�hlen
					}
					if (timer3 != tick3)
					{ //6 ms
						tick3 = timer3;
		...
		{ // Tuning Wheel
			...
			{ // 0,03 Sekunden
				...
				if ((SGUI.touch.xp > 170) &&
					(SGUI.touch.yp > 0))
				{ // touches left from 170 are ignored
					// touch is active
					active++;
					x = CalcAngle(SGUI.touch.xp, SGUI.touch.yp);
					if (x < 15000)
					{ // touch in the target field
						if ((active == 1) || (active < 0))
						{ // first touch after pause
							active = 2;
							OldAngle = x;
							if (circ != oldcirc)
							{ // change the tuning circle
								oldcirc = circ;
...
void create_ChildWindow_30(void)
{														 // *** Filters ***
...
// ** Routinen f�r Frequenzeingabe **_____________________________________________________________
void SendFreq()
{
	uint8_t i;
...
void btn_Freqfkt(bool aktiv)
{
	if (aktiv == true)
	{
		if (FrequChar[0] == 'A')
		{
			SetFreq((char *)&FrequCharA[0]); // Frequenz zur�ckschreiben
		}
		else
		{
...
void PrepFreqBtns(void)
{ // Prepare Frequency Buttons
	oldptr = bt8;
	oldpos = 8;
...
void frequA(bool aktiv)
{ // Button FrequA pressed
	if (aktiv == true)
	{
...
void frequB(bool aktiv)
{ // Button FrequB pressed
	if (aktiv == true)
	{
...
void Tune(bool aktiv)
{ // +++++ Tune command to Red Pitaya +++++
	uint8_t gaintun, xnr;
	if (aktiv == true)
	{
		tune = 1;
...
void Mute(bool aktiv)
{ // +++++ Mute command to Red Pitaya +++++
	if (aktiv == true)
	{
...
{ // +++++
	...
	{
	case 0:
		xnr = 0; // (exchange CWL <> CWU)
		break;
	case 1:
		xnr = 1;
		break;
	case 2:
		xnr = 3; // (exchange LSB <> USB)
		break;
	case 3:
		xnr = 2;
		break;
...
{ // returns angle
	...
	{ // central circle --> End of input/function
		if (oldcirc != 0)
			ShowVFOKnob(0);
		oldcirc = 0;
		return 20000;
...
int16_t CalcAngle2(int16_t x, int16_t y)
{ // returns angle
...
{				//													***************************************************************
  ...
  SGUI_GraphSetCHVisible(graph,0,true);*/
	...
	label2 = SGUI_LabelCreate(256, 0, 96, 23); // Feldst�rke
	...
	SGUI_ButtonSetText(btn9, (char *)FrequCharA);	   // uint16_t i; dunkelgrau  0x630C   0x076CE lindgr�n"A 59.123.456"
...
void SetColorBtn(SBUTTON_t *ptr, char n)
{
	...
	SGUI_ButtonSetColor(ptr, RGB_COL_BLACK, 0x076CE); // Cursor setzen (lindgr�n)
	if (oldptr == ptr)
		return;
	SGUI_ButtonSetColor(oldptr, RGB_COL_BLACK, RGB_COL_GREY); // alten Cursor l�schen
	oldptr = ptr;
	oldpos = n;
}
...
void FrequShift2(int32_t freqShift)
{
	...
	if ((10000 >= helpFreq) || (helpFreq >= 61440000))
		return;
...
uint16_t calcU(uint16_t messRoh)
{ // return: 10 * voltage
	double wertdbl;
...
void btn_Edit(bool aktiv)
{
	if (aktiv == true)
	{
		if (SGUI_ButtonIsAktiv(btn227))
			Index = 3;
		else if (SGUI_ButtonIsAktiv(btn225))
			Index = 1;
		else if (SGUI_ButtonIsAktiv(btn226))
			Index = 2;
		else if (SGUI_ButtonIsAktiv(btn224))
			Index = 0;
		else
			Index = 255;
		start = 1;
...
void btn_Send(bool aktiv)
{ // sends F1 or..or F4 text
