...
static int space_sent=false; ;	// for word space logic
...
// CW_SUCCESS is returned when there is a printable character. Nothing to do if we are in a tone
	...
	if (cw_receive_state == RS_IN_TONE){
		...
		if(duration > MAXTONE) {// permanent tone ?
			cw_receive_state = RS_IDLE;
			return ;
		}
		if(duration> 4 * cw_dot_length){// permanent tone or slower speed
...
// FARNSWOTH MOD HERE -->
		if(duration>8*cw_dot_length){// a longer pause
			duration = usec_diff(cw_keyup_timestamp, curr_timestamp);
			cw_receive_state = RS_IDLE;
...
// are idle, not inside of a character.
	if ((cw_receive_state != RS_IN_TONE)){
		...
		if(CW_pauseTime <= cw_noise_spike_threshold){//negative spike
...
// If the tone length is shorter than any noise cancelling
	...
	if(CW_TIMING_STATE!=CW_SYNCHRONIZED){// We are searching for two different times
		...
		if(CW_TIMING_STATE==CW_2ND_TIME_ELEMENT){
			if(duration>2*length1){//dit-dah
				Timecalc=(length1+duration)/2;
				if(Timecalc<=2*cw_noise_spike_threshold) return;
				two_dots=Timecalc;
				cw_dot_length=Timecalc/2;
				CodedMorse_sign=5;
			}
			else if(duration<length1/2){//dah-dit
				Timecalc=(length1+duration)/2;
				if(Timecalc<=2*cw_noise_spike_threshold) return;
				two_dots=Timecalc;
				cw_dot_length=Timecalc/2;
			...
			else if((3*duration<4*length1)&&(4*duration>3*length1)){// same length
				if(2*CW_pauseTime<duration){//dah-dah
	...
	if(element_counter>=7){// test,if more than 8 elements
		if((duration<two_dots)&&(CodedMorse_sign==0x80)){//ii correct Irrung
			decodedSign = 'j';
			ShowMorseSign();
			CodedMorse_sign=1;
			element_counter = 0;
...
// Dot = 0  code: 'A' -> CodedMorse_sign=0101b 'B' ->011000b
	if (duration > two_dots) {// a dash is anything longer than 2 dot times
...
void MakeMorseCode(){// fill the MCode table
uint16_t i;
for(i=0;i<=255;i++){
...
int8_t CodeMorse(uint8_t zeichen){// coding 1 char from keyboard or text memory F1.. F4
	...
	if(zeichen!=' '){
		...
		while((i<7)&&(overflow==0)){
			...
			if (code>=0x80){// 1 --> '-'
				...
				if (PSinPointer==PSoutPointer) overflow=1;
				else if(PSinPointer>=456)PSinPointer=0;
			}
			else PunktStrich[PSinPointer++]='.';//0 --> '.'
			if (PSinPointer==PSoutPointer) overflow=1;
			else if(PSinPointer>=456)PSinPointer=0;
			i++;
		}
...
char DecodeMorse(){// decoding 1 character/number from key input
uint8_t i=1, k;
char val;
do{
	val=PunktStrich[PSoutPointer++];
	if(PSoutPointer>=456) PSoutPointer=0;// ring buffer
} while(val==' ');
if(val=='.') i=2;
else if(val=='-')i=3;
for(k=7;k>=1;k--){
...
void SendMorsecode(void){
    ...
    if (PSinPointer!=PSoutPointer){
    	 if(gestartet==0){
    		 ...
    		 if(PSoutPointer>=456)PSoutPointer=0;
    		 if((zeichen=='.')||(zeichen=='-')) MorseKey(true);
    	 }
     }
    if(gestartet==1){
    	if(zeichen=='.'){
